...
| **Integer-width halfSockets non-manifold family** (`compartmentBuilder.scenario.manifold`: `1×6 halfSockets ±40` nm=76, `2×6 halfSockets ±40` nm=136, `2×6 2×2 crossing tilts` nm=140, `1×4 2×8-comps` nm=12 — all bnd=0) | algo/GFA | Distinct from the (closed) corner-crescent family: pure NON-MANIFOLD edges, zero boundary edges, integer-width bins only; counts identical pre/post the crescent fix (76↔77, 136↔137 = mesh noise), so pre-existing and un-masked by the analytic sockets (#1045) — the pre-loft crossing-tilts close (#1037-era) was measured on faceted sockets. First probe: fresh capture of the `1×6 halfSockets ±40` export chain (probe branch `probe/boolean-capture-2`), replay, and find the op emitting nm>0 — an edge used by >2 faces means a kept coincident/duplicate face, so check SD pairing and within-rank dups first, not classification. |
