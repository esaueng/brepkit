...
| **Per-face tessellation of wavy-band faces snap-skins the full parametric band** (the only residual of the now-CLOSED tangent-circle family) | operations/tessellate | The per-face route (`tessellate(topo, face, defl)` — no shared edge pool; feeds `classify_point`'s meshes) still skins a winding-chain band face as the full revolution between its v extremes, covering the dropped region. The SOLID path is correct (cycle-based structured band); volume, export, and parity consume the solid path. Repro: the `circle_outside_cone_box_fuse_is_watertight` solid, per-face mesh area 268.2 vs analytic 244.6. Fix shape: route the per-face path through the same cycle-rim sweep with locally sampled rims |
