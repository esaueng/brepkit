...
| **Cone ∪ box "circle outside" residual** (1 free boundary edge; the only surviving tangent-circle-family member) | algo/GFA | `diag_cone_box_tangency_sweep` "circle outside d=6 zb=6": the box is SMALLER than the section circle, so the section is 4 arcs meeting the box corners, a different topology from the tangency cases. The tangency family proper (2- and 4-tangency, cone and cylinder) is CLOSED — analytic, watertight, exact volume; pinned by `cone_union_box_should_be_analytic`, `cylinder_union_inscribed_box_is_analytic_watertight`, `two_tangency_box_fuse_is_analytic_watertight`. Closing machinery: duplicated-ring signature (cross-loop AND within-loop midpoint-keyed forms) → DCEL rescue with pre-split parallel arcs; co-endpoint midpoint splits (the sanctioned merge-key pattern) at DCEL input, adopted loops, and plane-arrangement section inputs (same-chord collapse); nearest-copy periodic UV unwrap (single-step could not recover multi-period jumps); arc-chain + unequal-count zipper band tessellator |
...
| **Ellipse sections spanning > π** are outside the shorter-arc contract (theoretical, no repro) | algo/GFA | Enforce the ≤ π split at ellipse-section emission, mirroring the circle emitter |
| **algo ray-cast classifier has no `Torus` arm** — torus faces fall to the flat Newell-polygon fallback | algo | Same parity class as the closed cone gap. Calibrated torus-box landscapes pin current behaviour, so re-probe before adding the arm |
