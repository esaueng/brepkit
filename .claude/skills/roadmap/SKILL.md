...
| **Per-face tessellation of wavy-band faces snap-skins the full parametric band** (the only residual of the now-CLOSED tangent-circle family) | operations/tessellate | The per-face route (`tessellate(topo, face, defl)` — no shared edge pool; feeds `classify_point`'s meshes) skins a winding-chain band face as the full revolution between its v extremes (area 268.2 vs analytic 244.6 on the `circle_outside_cone_box_fuse_is_watertight` solid; the −x lobe probe classifies Outside). The SOLID path is correct (cycle-based structured band); volume, export, and parity consume it. ATTEMPTED AND REVERTED: a cone arm mirroring the cylinder's boundary-CDT with edge-SAMPLED boundaries got 203.5/244.6 — the seam-twice cut-annulus wire pinches the UV polygon and the CDT drops a sector. Fix shape: reuse the solid path's cycle-rim zipper with LOCALLY sampled rims (sample each rim cycle's edges at deflection, sort by angle, zipper) instead of a boundary-CDT |
