...
| **v2 trimmer residuals** (3 remaining) | blend | CLOSED 2026-08-04: `create_blend_face` duplicate contact edges — the blend face now REUSES the trimmers' contact edges when endpoints match within weld (regress bnd 22 → 7, pin tightened to <= 7). Remaining: keep-side selection degenerate under tangency; `chamfer_v2` external tangent branch. End-cap notch CLOSED 2026-08-04: cross edges are the true end cross-section arcs and the caps are notched to share them (the caps previously COVERED the scooped corner — a volumetric error invisible to the free-edge count); chamfer contact reuse threaded alongside the fillet's. `crates/operations/tests/regress_blend_trim_neighbor_split.rs` |
