...
| **v2 trimmer residuals** (3 remaining) | blend | CLOSED 2026-08-04: `create_blend_face` duplicate contact edges — the blend face now REUSES the trimmers' contact edges when endpoints match within weld (regress bnd 22 → 7, pin tightened to <= 7). Remaining: keep-side selection degenerate under tangency; no end-cap notch trim; `chamfer_v2` external tangent branch (chamfer contact reuse threaded 2026-08-04 alongside the fillet's). `crates/operations/tests/regress_blend_trim_neighbor_split.rs` |
