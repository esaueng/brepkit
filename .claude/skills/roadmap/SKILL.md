...
| **Post-loft fractional-width halfSockets family** (`compartmentBuilder.scenario.manifold`: `1.5×6 halfSockets` baseline + ALL tilt variants → bnd=104 each, tilt-independent; `1×6`/`2×6` variants also fail; `1×4 2×8-comps` STL → nm=12) | algo/GFA | NEW geometry, not a regression: a #1043-only (pre-loft) kernel PASSES the same cases — the loft's analytic sockets (#1045) changed every bin base and un-masked this. bnd=104 constant across tilts = deterministic, tilt-independent single root at the socket×body interface, likely a variant the disconnected-loop twin fix doesn't cover (fractional 1.5-width bins have edge-clipped half-cell outlines — check outline↔boundary tangency/connectivity in the arrangement first). First probe: FRESH capture of the 1.5×6 baseline export chain with `probe/boolean-capture-2` (all old captures stale), then replay + DUMP_SPLIT/DUMP_ARR on the socket fuse. |
...
| `fuse_shelled_box_with_socket_loft` (`crates/operations/src/boolean/tests.rs`) | operations/loft + algo | Non-manifold edge at shelled-box + socket-loft fuse. The old "gated on a curve-preserving loft" framing is DEAD: the curve-preserving loft landed and the test still fails (euler != 2 with positive volume — re-measured on the combined tree 2026-07-07). Needs a fresh diagnosis of the fuse itself. Probe: run it `-- --ignored --nocapture` (prints F/E/V/euler + manifold issues). |
