...
|---|---|
| **#1488 baseplate perf, tool-side re-measure** | Kernel-side root closed (#1490: grazing plane-cone chains clipped before the O(n³) fit; plate_probe 4x4 2538→564ms). READY once ≥3.2.12 is on npm: re-run the issue's `profileBin -t "profile baseplates"` table on stock pins, then close or re-attack. Repro: `crates/operations/examples/plate_probe.rs`; guard: `tangent_graze_section_fit_is_clipped` |
