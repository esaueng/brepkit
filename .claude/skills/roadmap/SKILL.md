...
| **Export matrix drift: 3 cases newly leak under TOOL-side generator changes** (found 2026-08-05) | algo/GFA + tool | Re-probing the 4 historic export arms on the current kernel: 70/73, with `2x2 slotted no lip` (bnd 107-109), `2x2 mixed-detail per-cell half sockets` (bnd 259), and `3x3 O-shape + half sockets` (nm 8) failing. FULL KERNEL BISECT EXONERATES every 2026-08-04/05 engine PR: the same three failures reproduce byte-for-byte on the session-start kernel (73a4c2ce), so the tool's generator changes since the 73/73 measurement (#3223-#3227 era) altered these configurations' geometry into shapes the kernel has never handled. These are NEW scenario-coverage gaps in the slotted and per-cell/mixed-socket families. SLOTTED CAPTURED AND REDUCED (2026-08-05): `crates/io/tests/slotted_nolip_fuse_inmem.rs` — of the 10 booleans in the export chain, only the final socket-assembly fuse fails (clean F=56 slotted body + clean F=136 four-socket assembly abort with "open hole shell with 45 faces", the thick-wall abort family) and the fallback output carries the 107-109 boundary edges. BOTH HALF-SOCKETS CASES CAPTURED (2026-08-05): the `3x3 O-shape + half sockets` root is `crates/io/tests/oshape_socket_fuse_inmem.rs` — two clean 49-face socket pieces (one carrying 12 NURBS quarter-socket faces from the per-cell dispatch) abort with "open growth shell with 45 faces"; the chain's later 1022-planar-face fuse is its collateral. The `2x2 mixed-detail per-cell half sockets` case (bnd=259) replays ENTIRELY CLEAN through all nine booleans — its leak is POST-boolean (tessellation/export or an op class the boolean capture does not hook): triage that with a native tessellation boundary count of the final fused body before assuming geometry. baseStyles arm stays 26/26; kumiko-dividers stays in budget |
| **Mitsukude panel cut loses the lip-transition patch** (found 2026-08-04, the real root of the kumiko-dividers 183 s) | algo/GFA | `crates/io/tests/mitsukude_panel_cut_inmem.rs` (ignored ready-repro, 0.13 s): the raw GFA cut preserves every analytic surface in ~200 ms but leaves 8 free edges in ONE loop at the stacking-lip transition band (x 38.05/38.00 planes + r=0.05 arc band + 45-degree chamfer) where a strut pocket crosses the three-surface junction — the sub-face patch spanning it is lost, the ops gate rejects the result, and the mesh fallback burns 82 s and emits over=4. Loop ownership (FREE_EDGES=1, 2026-08-04): each free edge has exactly one user — the east-wall plane, the lip chamfer cone, and the corner cylinders — so the MISSING partners are the wall sub-faces bounded by the loop (at least two: the loop spans both wall planes and the cone). Window probe (BK_CLS=1, 2026-08-04): only 4 sub-faces classify inside the loop window — the body's three r=0.05 lip-band cylinder sub-faces (Inside, correctly dropped) and one tool plane at x=38.05 (Outside). NO body wall-plane sub-face is emitted there, so the kept tool pocket faces get no wall partner: the splitter never emits the wall patch, it is not classified out. Wire-touch probe (BK_CLS2) + point oracles (2026-08-04): the tool panel's END plane at x=38.05 is correctly dropped (ray-cast oracle: the body is genuinely void at its samples), and body material exists at x=38.025 beside it — so the missing partners are BODY sub-faces: the corner-sliver pieces of the lip-band cylinders (src 7/15/83) and wall planes (src 8/16/80/90) that the tool end plane should split off. Each of those body faces was split into only TWO pieces (pocket Inside + far Outside); the FF sections against the tool end plane either were not emitted on those faces or the splitter did not incorporate them, so the kept Outside pieces end short of x=38.05 and the 8-edge loop stays unpaired. FF-section probe (BK_SECS, 2026-08-04): EMISSION IS FINE — each lip-band cylinder (src 7/15/83) carries the x=38.05 axial ruling section (byte-identical to the free edges) plus the short crossing sections, yet the splitter produced only TWO pieces per cylinder: the corner sliver (beyond the tool end plane, still body material) was merged into the pocket region, classified Inside by its single sample, and dropped with it. RECLASSIFIED after the wire dump (BK_SPLITW/BK_CLS3, 2026-08-04): the splitter is NOT at fault — it produced 9 pieces for the src-7 cylinder, all registered and classified, and the kept Outside piece DOES carry the ruling edge. The structural fact: the tool panel END plane at x=38.05 is EXACTLY TANGENT to the r=0.05 lip-band cylinders (centre x=38.0 + r), the "sections" on the end plane are tangent rulings, and the body material pinches to ZERO width at each ruling — so no end-cap sub-face exists on either side, the kept cylinder piece's ruling edge has no partner, and the loop is the pinch line. This is the TANGENTIAL-CONTACT family (same class as the inscribed cylinder-box rulings and the cone-box tangency count map): the panel generator sizes the pocket to end exactly at the band tangent. Fix shape: pinch-aware handling at tangent rulings (collapse the zero-width wedge so the two tangent surfaces share the ruling edge, or weld the kept pieces across it), NOT a splitter section fix. The probes (BK_CLS/BK_CLS2/BK_CLS3/BK_SECS/BK_SPLITW) are committed and env-gated for the fix session. The 8-panel compound cut also aborts on a 4-face 0.57 mm^3 open hole-shell fragment (face-count guard `>= 4` treats it as a lump; a volume-relative gate was considered but the single-panel loop is the narrower entry point). Capture recipe: kernel-test monkey-patch, call 036 of the kumiko-dividers main arm |
| **Kumiko corner wedge: overlapping coaxial cut removes NOTHING and drops the coincident cap** (re-captured 2026-08-04; characterization corrected same day) | algo/GFA | `crates/io/tests/kumiko_corner_wedge_inmem.rs` (ignored ready-repro): on post-revolve-fix operands the coaxial wedge-strut cut runs ANALYTIC in 2 ms keeping both cylinders (the old all-planar fallback is gone) but is wrong twice over: the strut genuinely OVERLAPS the wedge (point-oracle verified; it protrudes through the wedge's y=0 cap plane) yet the result keeps the full 285.861 volume, and the coincident y=0 cap is dropped (F=6 -> F=5, free=4) — its interior sample lies ON the strut boundary, `classify_coincident_coplanar` defers straddlers, and ray-cast answers Inside. Coincident-contact family (with the mitsukude pinch). Section inventory (2026-08-04): the wedge's y=0 cap receives ZERO sections although the strut's coaxial cylinder walls cross it in vertical lines — the plane-contains-axis x coaxial-cylinder FF case emits nothing — and the few sections that ARE emitted (1 on each z-plane, 4 on one strut face) each "split into 1 sub-faces", i.e. the splitter declines them. This is the [[project_coaxial-samedomain-frontier]] in its purest 12-face form. FF pair map (BK_FF_TRACE, 2026-08-04): the only 4 emitted sections all pair wedge faces 1-4 with ONE strut plane (the far angular cap; the strut's near cap is NOT coincident with the wedge's y=0 cap — its caps are slanted, y ranges [-0.57,-0.11] and [0.22,1.10], correctly AABB-rejected). MECHANISM FOUND (BK_RAWC + EF logs, 2026-08-04, after two refuted framings): every AABB rejection is geometrically CORRECT — the strut RADIALLY ENGULFS the wedge (cylinders at r 1.04/5.19 bracket the wedge's 1.55..4.75), so the y=0 cap lies wholly inside the strut and its removal is right, and the only genuine intersection curves are the 4 sections against the strut's far angular cap, ALL EMITTED. The defect surface: all four sections fail to split their faces ("split into 1 sub-faces") and nothing is cut. Two EF crossings are dropped ("edge Id(15) at t=0.031 — outside face Id(2) boundary") — but CAUTION before blaming chord containment: `build_face_containment` already carries a sagitta margin for curved edges, and the dropped edges may be the strut's r=1.04/5.19 cylinder edges piercing z=20.8 genuinely OUTSIDE the wedge sector (correct drops). ROOT ARITHMETIC (BK_SPLITW section dump, 2026-08-04): face 2 receives ONE Line section whose endpoints sit at r=4.000 and r=1.306 — EXACTLY where the section line crosses the SINGLE CHORDS of the face's two 90-degree NURBS arcs (4.75/sqrt(2)/cos(32.9deg)=4.00; 1.096/cos(32.9deg)=1.306), instead of the true arc crossings at r=4.75/1.55. So the section endpoints float 0.75 mm inside the face / overshoot the inner arc, the splitter cannot anchor them to the boundary, and it declines (1 piece). EXONERATED: `restrict_curves_to_faces`'s FaceWindow polygon samples curved edges at 16 points (sagitta 0.006 mm) — it is NOT the chord source. FIND (narrowed 2026-08-04): `clip_line_to_face` is EXONERATED — it bails Indeterminate on any non-Line boundary edge, so the wedge's arc faces are never polygon-clipped there. `build_section_edges` reads endpoints from the ARENA curve via `curve_endpoints`, so the chord crossings were paved upstream: prime suspect is the EE/EF machinery intersecting the FF section line with the wedge's NurbsCurve boundary edges via their CHORD (endpoints), placing the curve's extreme paves at r=4.00/1.31. CRITICAL ASYMMETRY (EF log, 2026-08-04): EF finds and KEEPS the four TRUE arc crossings (wedge arc edges x strut far cap, all at t=0.1436 — the exact NURBS crossings; the earlier-logged drops are the strut cylinder edges outside the sector, correct). So the true junctions EXIST as paves on the wedge's boundary arcs; only the FF curve's OWN end paves sit at the chord crossings (r=4.000/1.306, chord arithmetic exact to 4 decimals). CLIP FIXED (PR #1343, 2026-08-05; successor to the closed #1341): `clip_line_to_face_boundary` now intersects sections with NurbsCurve boundary edges by sampled sign-change bisection in the face plane (bounded cost; honeycomb stays at 0.75 s where bezier clipping took 115.6 s), and a chord crossing is dropped as a phantom border ONLY when its segment's true-arc hit lands within the section range (dropping it unconditionally broke the honeycomb residual pins — measured, narrowed). The wedge section is now ONE clean arc-to-arc piece (r 4.7497 to 1.5504). SPLITTER LAYER FOUND AND NEARLY CLOSED (branch `fix/nurbs-boundary-image-expansion`, parked 2026-08-05): the splitter declined because the boundary-image expansion was Line-only — NURBS arcs never arrive pre-split at their pave junctions, so the section cannot anchor. The parked branch extends `boundary_edges_to_pcurve_with_images` to NURBS edges, gated THREE ways after two measured foil breaks: circle-likeness (5-sample circumcircle fit — marched free-form NURBS like the snapClip deepened-notch walls broke under expansion and are never circles), weld-coincidence of the junction with a section anchor, and planar hole-free faces. With it the wedge z-planes split and the cut removes material for the FIRST time (285.9 -> 276.2 stable; -> 247.5 watertight F=6 all-analytic when the cylinders also pair). BLOCKER: watertightness is knife-edge across BUILDS of identical source (free=0 vs free=8, each stable 6/6 within a binary — sub-tolerance float sensitivity in the cylinder-side anchoring). Harden that weave (widen the cylinder-side junction matching to the weld band instead of exact vertex identity is the first candidate), then activate the wedge volume pin (240..280 band, written and re-ignored in the fixture) |
