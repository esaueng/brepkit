...
| **hashbrown capacity-overflow panic (#1517 root b)** | `[brepkit] panic: Hash table capacity overflow` in the lid-retention-magnet chain. The 4 `recursive use of an object` errors are SECONDARY — wasm32 is `panic=abort` so a trap strands the wasm-bindgen borrow flag. Shape is a runaway hash insertion, not a bad `with_capacity` (`crates/operations/src/copy.rs` uses `saturating_add`); wasm32's 32-bit `usize` hits the ceiling long before native, so a native repro may not panic at all. Test the same hypothesis against the 14 timeouts |
