...
| **User-reported (2026-08-06): honeycomb wall pattern looks wrong near corners** | tool + kernel, unreduced | No repro yet. Reproduce tool-side (binStyles wallPattern honeycomb), inspect the geometry near a bin corner, decide tool-side pattern layout vs kernel boolean/tessellation, then reduce to a native fixture if kernel-side (capture recipes in parity-benchmarking) |
| **User-reported (2026-08-06): triangle wall pattern arrangement is wrong** | tool + kernel, unreduced | No repro yet. Likely the tool's pattern-layout math (arrangement, not geometry corruption) — verify tool-side first before blaming the kernel; a fast-failing or visually-misplaced-but-manifold result is pre-geometry |
