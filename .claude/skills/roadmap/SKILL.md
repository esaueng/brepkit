...
| **hashbrown capacity-overflow panic (#1517 root b)** | CONTAINED, not cured. The unbounded container was the CDT's: `recover_edge_depth`'s crossing split reset the bisection budget, so crossing and bisection recursion together had no bound and grew `constraints`/`dup_grid`/`vertices` freely. Bounded in #1520 by a recursion-level cap and a per-triangulation recovery-insert budget, which turns the kernel-wide abort into a local `ConvergenceFailure`. The GEOMETRY that drives a face into runaway recovery is still unidentified; it now fails observably instead of aborting, which is what makes it findable. Calibration: recovery mints one point per crossing pair and ZERO across the 823 `brepkit-operations` tests |
