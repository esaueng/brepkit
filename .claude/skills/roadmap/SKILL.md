...
| **Quadric ∪ box with a 2-tangency section circle** — the surviving half of the tangent-circle family (4 → 4 free edges, `diag_tangency_count` "2 tangent walls") | algo/GFA | The 4-tangency case is CLOSED (analytic, watertight, exact volume; `cone_union_box_should_be_analytic` + `cylinder_union_inscribed_box_is_analytic_watertight` pin it): duplicated-ring signature → DCEL rescue on cone/cylinder laterals, co-endpoint half-rim midpoint splits (the sanctioned merge-key pattern), and an arc-chain + unequal-count zipper band tessellator. The 2-tangency case splits the circle into TWO arcs and fails differently (open chain, 4 free edges) — it needs its own trigger; start from `diag_tangency_count` |
