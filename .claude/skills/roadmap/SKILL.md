...
|---|---|
| **#1488 baseplate perf, tool-side re-measure** | TWO kernel roots closed: #1490 (grazing plane-cone chains fed the O(n³) fit) and #1495 (edge-tangent PREVIEW pockets made every cluster fuse mesh-fallback; the blob then poisoned every later boolean — the non-monotonic corner-clip anomaly is its collateral and dies with it). READY once the release after 3.2.12 is on npm: re-run the issue's per-stage breakdown (gridfinity-layout-tool#3348) on stock pins, then close. Repros: `crates/operations/examples/plate_probe.rs` (export profile), captured preview operands via the brepjs probe; guards: `tangent_graze_section_fit_is_clipped`, `compound_cut_edge_tangent_tools_stays_analytic` |
