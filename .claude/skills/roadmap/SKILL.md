...
| **Cone ∪ box tangent section circle** — the last primitive-boolean fallback | algo/GFA | `boolean::tests::cone_union_box_should_be_analytic` + the `diag_*_tangency*` `#[ignore]`d probes; their doc comments carry the map. Trigger is the COUNT of tangency points (0/1 clean, 2 → 4 free, 4 → 4 nm). Fix shape: add "inner wire duplicating the outer" as a `greedy_broken` trigger (covers the 4-wall case only; the 2-wall case needs its own) |
...
| **Disjoint `Cut` is not short-circuited** — the ops shortcut detects only containment, so a non-touching tool still routes through GFA and then the mesh fallback | operations | Cheap win; seen on kumiko corner struts |
