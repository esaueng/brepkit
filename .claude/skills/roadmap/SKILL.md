...
|---|---|---|
| **Compartment manifold residuals** (re-scoped 2026-07-07 after the EF tangent-endpoint fix landed; 7/13 pass) | algo/GFA + unknown | The lip-corner "doubled faces" root is CLOSED: it was a grazing EF contact minting a vertex 3.1µm from the arc's endpoint (fix: angle-scaled endpoint window in `phase_ef`; fixture `crates/io/tests/lipcorner_tangent_inmem.rs`). Remaining 6 failures are FURTHER small defects of the same near-duplicate/sliver class at other features: `1×4 2×8-comps` = 4 NM STL edges (was 8), `top-row merged`, and 4 tilted-divider/halfSockets cases (4–64 defects, several reduced by the fix). Probe recipe: instrumented-kernel capture in `all` mode (now hooks `boolean_with_evolution` — the tool's export fuses ALL go through the provenance path, invisible to a `boolean()`-only hook) + `VERTEX_WATCH` backtrace trap in `Vertex::new` (probe branch). |
