...
Head-to-head (wasm vs wasm, 2026-08-05, current kernel hash-verified overlaid into the bench harness — NOTE: the harness's vitest resolve.alias does NOT reach the CJS require path, so overlay node_modules and hash-verify; an alias-only run silently benches the installed kernel): brepkit leads every row — cut(box,cyl) 2.6x, fuse(box,box) 88x, meshing 1.4-3.8x, transforms ~4x, volume 2.3x, STEP export 14.7x. Equivalence-verified rows only: fuse is exact (1000.000 both); cut carries the 0.18% geometry deviation pinned below; the intersect row is DISQUALIFIED (wrong region, pinned below). Where parity stands: gridfinity bin parity reached; all four primitive-boolean
...
| **Bench-found: intersect(corner box, center sphere) keeps the wrong sphere region** (2026-08-05) | algo/GFA | `bench_equiv_intersect_box_corner_sphere_is_the_octant` (ignored ready-repro): vol 1304.8 vs exact octant 268.083; every probe point classifies Outside including inside the true octant; oriented volume 1148.8 disagrees with the 1304.8 magnitude (malformed boundary) while edges stay paired. Three coordinate planes through the sphere CENTER = the chord-discretized-equator sphere-split family — TERMINAL-adjacent (the general UV-space sphere arrangement splitter is the named missing primitive), so scope any fix to the center-plane special case. Found by bench output verification: the "236x faster" intersect row was doing the wrong work |
| **Bench-found: cut(corner box, corner cylinder) removes 2.3% too much** (2026-08-05) | algo/GFA | `bench_equiv_cut_box_corner_cylinder_volume_is_exact` (ignored ready-repro): 927.6577 vs exact 929.3141, deflection-INVARIANT (not a tessellation artifact; the primitive cylinder measures exactly 565.4867 through the same path). Face census correct (6 planes + 1 cylinder) — the trimmed geometry deviates. First probe: per-face area audit against closed forms to localize which face's trim is off |
