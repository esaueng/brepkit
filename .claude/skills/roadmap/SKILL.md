...
|---|---|---|
| **Tangent section circle on a quadric lateral vs. box walls** — surfaces in the census as `cone ∪ box`, the ONLY remaining primitive-boolean mesh fallback (ready-repro `boolean::tests::cone_union_box_should_be_analytic` + sweeps `diag_cone_box_tangency_sweep` / `diag_cylinder_box_tangency`, all `#[ignore]`) | algo/GFA | Root-mapped 2026-07-24. NOT cone-specific: a plain CYLINDER r=4 fused with a tangent d=8 box fails identically (4 nm) and is clean at d=10 — so this is the general tangential-contact class, same family as the intwidth-tangency closure, and gridfinity geometry is full of it. `make_cone(6,2,12)` is r=4 at z=6 where the 8x8 box's bottom sits, so the section circle is INSCRIBED in the box square and TANGENT to all four walls (the recurring tangential-contact class). Raw GFA nearly succeeds: F=10, 1 cone + 9 planes, free=0, rejected only for 4 non-manifold edges. The BOX side is CORRECT — the pinched square-minus-inscribed-circle tiles into 4 curvilinear triangles and each wall splits at its tangency point. The CONE side loses the z=0 base rim: both cone plane faces vanish (z=12 top disc correctly absorbed, but the z=0 base disc r=6 is nowhere near the box and must survive), and the cone lateral's single inner wire repeats its OWN outer wire (the same four z=6 arc edge ids) instead of the base rim — hence each arc used 3x. One missing piece, two symptoms: the remainder is closed by duplicating the wrong rim. TANGENCY IS THE TRIGGER: two independently chosen tangent cone configs (d=8/zb=6 and d=6/zb=9) fail IDENTICALLY (F=10, z0 disc dropped, 4 nm), while the same fuse with the section circle STRICTLY INSIDE the box is perfectly clean (F=8, validate ok). FAILURE BAND IS SUB-TOLERANCE (`diag_tangency_epsilon_band`, cylinder r=4 vs box half-width 4+eps): eps=-1e-3/-1e-5 (gross crossing) CLEAN, eps=-1e-7 10 free, eps=-1e-9 6 nm + 7 free, eps=0 4 nm, eps=+1e-9 and wider all CLEAN. So gross crossing and any clearance both work; only sub-tolerance penetration through exact tangency breaks, a band of ~1e-7 = the linear tolerance. That is the intwidth-tangency signature, whose near-tangent root collapse in `Circle3D::intersect_segment` already ships — so the residual bug is DOWNSTREAM of the collapse, in how a section circle split at pure tangency points is turned into regions. (A gross-crossing CONE case d=6/zb=6 does give 1 free edge, but the cylinder is clean there, so treat that as taper-specific and separate.) MECHANISM MEASURED: `Circle3D::intersect_segment` (r=4 circle vs a vertical segment at x) returns 2 hits at x=3.999, 0 at x=4.001, and exactly **1** at BOTH x=4.0 (true tangency) and x=3.9999999 (sub-tolerance crossing) — the intwidth near-tangent collapse firing, which is correct in itself. So each of the 4 walls contributes ONE hit, `closed_circle_boundary_crossings` splits the section circle at 4 TOUCH points, and the consumer treats them as crossings. That is exactly why the failure band equals the collapse's range. COUNT IS THE REAL TRIGGER (`diag_tangency_count`): 0 or **1** tangent wall is CLEAN; 2 gives 4 FREE edges; 4 gives 4 nm. One touch-split leaves the closed circle a single closed edge (P->P) and is handled; two or more turn it into a CHAIN of arcs the quadric side cannot close. CAUTION — the 2-wall and 4-wall cases are NOT confirmed to be one root: at 4 walls the quadric face SURVIVES mis-wired (outer wire == inner wire), while at 2 walls it is DROPPED ENTIRELY (F=8, all planar, the whole lower cylinder missing, box bottom left with used-once circle and line edges). Same trigger, materially different failures; treat them as two cases until shown otherwise. Fix must live downstream: the box bottom NEEDS those splits (its square-minus-disc region is genuinely pinched there), so do not un-split — make the quadric side reassemble the 4 arcs as one closed rim. FIX SHAPE (concrete): the existing rescue `split_cylinder_band_by_arrangement` is gated on `greedy_broken` (face_splitter/mod.rs ~5007), and it never fires here because the greedy result LOOKS structurally valid — the cone/cylinder face's outer wire is {16,22,23,24} and its inner wire is the SAME edge ids {23,22,16,24}, so nothing sets the broken flag. Add "an inner wire whose edge set duplicates the outer wire's" as another `greedy_broken` trigger: that is keyed to a DEMONSTRATED failure signature, so it cannot demote a working case — the same cheap route past the splitter's mutual calibrations that `loops_have_out_and_back` took for label-sockets. DETECTOR PRE-VALIDATED (env-gated probe in `validate_boolean_result`, since reverted): the duplicate-wire signature fires on the 4-tangent-wall case and on NOTHING ELSE across the whole brepkit-io fixture suite or the brepkit-operations lib tests — so it is safe by construction on the current foils. BUT IT IS INCOMPLETE: the 2-tangent-wall variant (4 free edges) does NOT carry the signature, so it needs its own trigger. |
