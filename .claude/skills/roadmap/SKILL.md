...
| **Lid magnet-post corner fuse (#1517 root b): the crash AND the 14 timeouts** | ONE root, not two symptoms. A post cylinder (r=4 at (-118.5,76.5), z=-2.8..-0.7) straddles a lid corner and pokes through BOTH walls x=-122 and y=80; the lid's z=-0.8 ledge cuts its cross-section into arcs. Raw GFA stays analytic (47 faces, 4 cone/17 cyl) but leaves **6 free edges**: the ledge plane and the post cylinder each keep DIFFERENT arcs of the same circle, so the two surfaces are split against each other inconsistently rather than one failing to split. Gate rejects (euler=-1) then mesh fallback turns 44 exact faces into 305 flat ones; the next ~24 post fuses compound the blob to 8852 faces; call 248 never returns, and after ~12 min the lookup table cannot grow and the kernel aborts. Repro `crates/io/tests/lidpost_fuse_inmem.rs` (**5ms**, ignored pin `lidpost_fuse_is_closed_and_analytic`); `FREE_EDGES=1 replay_pair` dumps the arcs. CONTROL (`lidpost_single_wall_control_fuses_closed_and_analytic`, active): the same lid with the opposite-corner post at (+122,-80) is clean (48 faces, 0 free). NOT a mirror twin, so not a one-variable comparison: `POINT_IN` at z=-1.800 finds material from x=-122.000 on one side but only from about x=+123 on the other, even though a wall plane at x=+122.000 exists and does cut sections. That asymmetry is unexplained and is the next thing to pin down. It crosses two walls too and yields the SAME seven FF sections (three duplicate ceiling circles at z=-0.800 + four wall lines), so neither the two-wall corner nor the duplicate sections is the discriminant, and the test's old 'ONE wall' name is a MISNOMER. The one measured difference is seam placement: failing crossings 61.05/118.95/151.05/208.95 leave the seam at 0 deg outside every crossing sector, control crossings 28.95/331.05/241.05/298.95 put the seam inside the x-wall sector. The spur anchors at that seam. Hypothesis, not result. NEXT LEAD (structural, in the #1517 thread): the post cylinder's outer wire carries a SPUR — `e24`/`e25` each appear twice in the same wire with the full z=-0.700 rim circle at the tip, and that rim is one of the 6 free edges. Walls, bottom cap and ceiling all trim correctly, so it is one face. The two sectors the cylinder keeps up to z=-0.800 both classify INSIDE the lid (points on the cylinder at 90 deg and 180 deg), i.e. they should have been dropped, and the cap of the same post is trimmed the other way. Suspect the sub-face interior sample landing on the seam at (-114.500,76.500), which is where the spur anchors. RED HERRING: `plane_internal_line_loops` logs 'not strictly interior' at 2.8e-14/8.9e-16/0 here, which is correct behaviour since those sections do touch the boundary. TRAP: a walls-and-ceiling mental model of this lid is WRONG — (-118.5,73.0,-1.8) sits inside the x=+-122/y=+-80 rectangle and still classifies Inside, so reason from `POINT_IN`, not from the face list. CDT constraint recovery was separately found unbounded and hardened in #1520, but it was not this root |
