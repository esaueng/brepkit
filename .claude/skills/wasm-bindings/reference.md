...
`crates/wasm-macros/src/lib.rs` contains a complete `#[wasm_binding]` attribute macro (js_name plus a `recoverable` mode with snapshot restore) but it is unwired: `remus-wasm-macros` is not in `crates/wasm/Cargo.toml` and `rg -n 'wasm_binding' crates/wasm/src` finds nothing. Treat it as a possible future consolidation, not the recipe.
