...
//! reversed when the solid carries a NURBS face.
//!
...
//! trimmed NURBS patch need not do.
//!
//! Filleting mirror-image corner columns of an L-blank produces mirror-image
//! solids whose NURBS blend walls land on opposite sides of that test, so the
//! two measured 12 mm³ apart: 49.03 and 36.68 where the closed mesh gives both
//! the same 34.7. The geometry was never at fault — only the measurement.
...
#[test]
fn nurbs_blend_volume_is_orientation_independent() {
    ...
    for cx in [0.0, 80.0] {
        ...
        // The v1 rolling-ball engine is what emits a NURBS blend wall here;
        // that is the shape this regression is about, so call it directly.
        #[allow(deprecated)]
        let out = brepkit_operations::fillet::fillet_rolling_ball(&mut topo, solid, &[seed], R)
            .expect("fillet");

        // The premise: without a NURBS face the solid takes a different
        // volume path entirely and the test proves nothing.
        saw_nurbs |= brepkit_topology::explorer::solid_faces(&topo, out)
            .expect("faces")
            .into_iter()
            .any(|f| matches!(topo.face(f).expect("face").surface(), FaceSurface::Nurbs(_)));

        let after = solid_volume(&topo, out, 0.01).expect("after");
        measured.push(before - after);
    }

    assert!(saw_nurbs, "expected a NURBS blend wall on this shape");
