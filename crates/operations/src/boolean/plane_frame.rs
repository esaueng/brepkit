...
impl PlaneFrame {
    ...
    pub fn from_normal_and_point(normal: Vec3, origin: Point3) -> Self {
        ...
        // u = normalize(normal × seed) — perpendicular to normal
        let u_raw = normal.cross(seed);
        let u_axis = u_raw.normalize().unwrap_or(Vec3::new(1.0, 0.0, 0.0));
        // v = normal × u — completes the right-handed frame
...
mod tests {
    ...
    fn xy_plane_roundtrips() {
        ...
        // UV coordinates depend on the chosen frame, but the 2D distance
        // from origin must equal the 3D distance: |(3,4,0)| = 5.
        let dist_2d = (p2.x() * p2.x() + p2.y() * p2.y()).sqrt();
        assert!((dist_2d - 5.0).abs() < 1e-10);
        // Roundtrip must recover the original point.
    ...
    fn tilted_plane_roundtrips() {
        ...
        // A point on the plane: origin + 2*u_axis + 3*v_axis
    ...
    fn from_plane_face_uses_first_vertex() {
        ...
        // The first vertex should project to (0, 0) since it's the origin
