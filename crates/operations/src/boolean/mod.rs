...
thread_local! {
    ...
    #[cfg(test)]
    static THREAD_MESH_FALLBACK_COUNT: std::cell::Cell<u64> = const { std::cell::Cell::new(0) };
...
#[cfg(test)]
fn thread_mesh_fallback_count() -> u64 {
    THREAD_MESH_FALLBACK_COUNT.with(std::cell::Cell::get)
}
...
pub fn boolean(
    ...
    if LAST_USED_MESH_FALLBACK.with(std::cell::Cell::take) {
        ...
        #[cfg(test)]
        THREAD_MESH_FALLBACK_COUNT.with(|count| count.set(count.get() + 1));
    }
    result
}
...
fn boolean_inner(
    ...
    if flatten_a || flatten_b {
        ...
        if flatten_settled {
            let working_result = boolean_inner(&mut working, op, working_a, working_b)?;
...
fn intersect_multi_region_semantically_safe(
    ...
    let saved_fallback = LAST_USED_MESH_FALLBACK.with(std::cell::Cell::get);
    let volume_audit = (|| -> Result<IntersectionVolumeAudit, crate::OperationsError> {
        ...
        let a_minus_b = boolean_inner(&mut audit, BooleanOp::Cut, a, b)?;
        let a_cut_volume = intersection_audit_mesh_volumes(&audit, a_minus_b)?;
        let b_minus_a = boolean_inner(&mut audit, BooleanOp::Cut, b, a)?;
    ...
    LAST_USED_MESH_FALLBACK.with(|flag| flag.set(saved_fallback));
