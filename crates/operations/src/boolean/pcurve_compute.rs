...
/// Evaluate a 3D edge curve at parameter t ∈ [0, 1].
///
...
mod tests {
    ...
    fn unwrap_periodic_removes_seam_jump() {
        let mut pts = vec![
            Point2::new(6.0, 0.0),
            Point2::new(6.2, 0.0),
            Point2::new(0.1, 0.0), // Jump from ~6.2 to ~0.1 (crossed 2π)
            Point2::new(0.3, 0.0),
        ];
    ...
    fn collinearity_detection() {
        ...
        let curve_pts = vec![
            Point2::new(0.0, 0.0),
            Point2::new(1.0, 1.0),
            Point2::new(2.0, 0.0), // Off the line
            Point2::new(3.0, 3.0),
        ];
    ...
    fn pcurve_line_on_cylinder_is_vertical() {
        // A ruling line along a cylinder's v-axis should produce a vertical
        // line in UV space (constant u, varying v).
        ...
        // Point on cylinder at u=0: (1, 0, 0). At u=0, v=5: (1, 0, 5).
        ...
        // Should be a Line2D (collinear in UV).
        assert!(
    ...
    fn pcurve_circle_on_cylinder_is_horizontal() {
        // An equatorial circle on a cylinder should produce a horizontal
        // line in UV (varying u, constant v).
        let cyl = brepkit_math::surfaces::CylindricalSurface::new(
            Point3::new(0.0, 0.0, 0.0),
            Vec3::new(0.0, 0.0, 1.0),
            1.0,
