...
pub fn remove_pendant_sections(
    ...
    // A min-heap preserves the old deterministic lowest-source-first peel,
    // while incremental incidence counts ensure every edge is visited only
    // when it is removed rather than rescanning the full soup each round.
