...
| Evolution | Face provenance (booleans, blends, patterns) | Beta | Retained; scope widened, label unchanged. Exact construction-derived provenance now covers booleans (GFA face origins), fillet and chamfer on the walking builder, and patterns (a copy's own face map), and `EvolutionMap::origin` distinguishes that from inference. The geometric matcher behind the remaining routes is now scale-relative — it previously answered three different things for one body at 1x, 1000x and 0.001x, at 0.001x binding a consumed face to a surviving one — and refuses ambiguity into an `unresolved` bucket rather than resolving it. A tie counts as ambiguity only when the output could actually be one of the tied inputs: an output parallel to none of them cannot be any of them re-trimmed, so it is new geometry built between them and is recorded as `generated` from each — the same answer the walking builder's construction record gives for the same band, so the two engines behind fillet and chamfer no longer disagree about what a blend face descends from. Every result face of a fillet, chamfer or boolean is now claimed by `modified` or `generated`, enforced as a set equality rather than a face count. Still Beta: offset, shell, draft, split, defeature and the direct edits produce no provenance at all; the planar rolling-ball fillet/chamfer rebuilds keep no record, so their maps stay inferences even where they are now complete; and there is no edge or vertex provenance. |
