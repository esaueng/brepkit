...
| PERF-001 | P1 | repeated boolean determinism | The tracked 64-cut diagnostic documents remaining `HashMap` iteration nondeterminism: two identical runs can diverge from the first cut and occasionally enter a roughly six-minute mesh-fallback path. | `cargo test --release -p brepkit-operations --test perf_64cut_determinism -- --ignored --nocapture` is the explicit diagnostic; it is intentionally excluded from normal CI while it remains failing. | Open. Replace remaining unordered iteration in topology-affecting paths, convert the diagnostic into a required passing gate, and establish a bounded fallback budget. |
...
`BOOL-002`, `PERF-001`, and `OPS-001` remain open P0/P1 release blockers, and
fork-hosted CI has not yet supplied independent evidence. See the stability
